...
|---------------|----------------|-------------|---------|
| RAM | IMemoryBus (直接) | 0x00000000-0x001FFFFF | なし |
| BIOS | IBios | 0x1F000000-0x1F07FFFF | なし |
...
| GPU | IGpu | 0x1F801810-0x1F801814 | IRQ1 |
| MDEC | IMdec | 0x1F801820-0x1F801824 | なし |
| SPU | ISpu | 0x1F801C00-0x1F801DFF | IRQ8 |
...
│ 0x1F000000-0x1F07FFFF: BIOS        │
│ 0x1F800000-0x1F801FFF: HW Regs    │
...
│ 0xFFFE0000-0xFFFFFFFF: Kernel RAM  │
└─────────────────────────────────────┘
```
...
| チャンネル | 用途 | 方向 | 同期モード |
|-----------|------|------|-----------|
| 0: MDECin | MDEC入力 | ToRam | SyncToBlocks |
| 1: MDECout | MDEC出力 | FromRam | SyncToBlocks |
| 2: GPU | 描画 | 双方向 | SyncToDmaRequest |
| 3: CD-ROM | セクタ読み取り | FromRam | SyncToDmaRequest |
| 4: SPU | 音声データ | 双方向 | SyncToDmaRequest |
| 5: PIO | 拡張ポート | 双方向 | SyncToDmaRequest |
| 6: OTC | リバースクリア | FromRam | Immediate |
...
- **GPUREAD**: GP0/GP1 の結果読み取り (GPUSTAT)
- **VBlank**: 垂直帰線時に IRQ1 を発火
...
- **IRQ8**: サウンドバッファがIRQアドレスをクロスした時に発火
...
IRQ5: Timer 1      (Timer)
IRQ6: Timer 2      (Timer)
IRQ7: Controller   (Controller/Memory Card)
IRQ8: SPU          (SPU)
IRQ9: PIO          (Expansion)
```

- **I_STAT (0x1F801070)**: 割り込みステータス (write-1-to-clear)
...
- **MODE レジスタ**: カウント開始/停止, 割り込み許可, ターゲット/オーバーフロー
- **COUNT レジスタ**: 現在のカウンタ値
- **TARGET レジスタ**: ターゲット値
