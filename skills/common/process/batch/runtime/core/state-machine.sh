...
_sm_valid_issue_transition() {
    ...
    case "$_from" in
        SUBAGENT_STARTING)
            case "$_to" in READY_FOR_NATIVE_DISPATCH|SUBAGENT_RUNNING|SUBAGENT_RETRYING|SUBAGENT_FAILED) return 0 ;; esac ;;
...
_sm_get_valid_issue_transitions() {
    case "$1" in
        SUBAGENT_STARTING) echo "READY_FOR_NATIVE_DISPATCH SUBAGENT_RUNNING SUBAGENT_RETRYING SUBAGENT_FAILED" ;;
