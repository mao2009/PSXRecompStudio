...
/// <see cref="MipsToIrLowerer"/>, and no indirect jump (JR/JALR) is included. This is
...
public static class RealRomCandidateSelector
{
    ...
    /// indirect jump is encountered. Returns <see langword="null"/> only when
    /// <paramref name="startAddress"/> is not present in <paramref name="orderedInstructions"/>.
    /// </summary>
        ...
        int maxWindowInstructions = DefaultMaxWindowInstructions)
    {
        ...
        while (true)
        {
            ...
            {
                return Build(startAddress, acceptedWords, accepted, RealRomCandidateStopReason.MaxWindowReached, null, "Maximum candidate window reached.");
            }

            if (i >= orderedInstructions.Count)
            {
                return Build(startAddress, acceptedWords, accepted, RealRomCandidateStopReason.EndOfDecodedInstructions, null, "No more decoded instructions.");
            }

            var current = orderedInstructions[i];
            if (accepted.Count > 0 && current.Address != accepted[^1].EntryPc + 4)
            {
                return Build(startAddress, acceptedWords, accepted, RealRomCandidateStopReason.NonContiguousInstructions, current.Address,
                    $"Decoded instructions are not contiguous at 0x{current.Address:X8}.");
            }

            var decoded = R3000aDecoder.Decode(current.RawWord);
            if (decoded.Opcode is R3000aOpcode.Jr or R3000aOpcode.Jalr)
            {
                return Build(startAddress, acceptedWords, accepted, RealRomCandidateStopReason.IndirectJumpExcluded, current.Address,
                    $"Indirect jump '{decoded.Opcode}' at 0x{current.Address:X8} is excluded by the #225 candidate policy.");
            }
            ...
            if (decoded.DelaySlot != R3000aDelaySlotKind.None)
            {
                ...
                {
                    return Build(startAddress, acceptedWords, accepted, RealRomCandidateStopReason.MissingDelaySlot, current.Address,
                        $"'{decoded.Opcode}' at 0x{current.Address:X8} has no contiguous delay-slot instruction.");
                }
                ...
                {
                    return Build(startAddress, acceptedWords, accepted, RealRomCandidateStopReason.IndirectJumpExcluded, delaySlotEntry.Address,
                        $"Indirect jump '{delaySlotDecoded.Opcode}' in the delay slot at 0x{delaySlotEntry.Address:X8} is excluded by the #225 candidate policy.");
                }
            ...
            {
                return Build(startAddress, acceptedWords, accepted, RealRomCandidateStopReason.UnsupportedInstruction, current.Address, ex.Message);
            }
...
public sealed record RealRomFunctionProvenance
{
    ...
    /// <summary>Always <see langword="false"/> for a candidate <see cref="RealRomCandidateSelector"/> accepted: unresolved dependencies are excluded by construction, never carried through silently.</summary>
