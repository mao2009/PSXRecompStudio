...
| Recompiler (MIPS→IR lowering, pure GPR subset) | Implemented (Phase 2A — in PSXRecomp.Core) |
...
**Recompiler.** PSXRecompStudio's ultimate goal is static recompilation. The backend-agnostic IR model and shared state contract are now implemented in `PSXRecomp.Core.Recompiler`. Phase 2A adds `MipsToIrLowerer`, which translates a pure-GPR subset of decoded R3000A instructions (NOP, ADDU, SUBU, ADDIU, LUI, AND, OR, XOR, NOR, SLL, SRL, SRA) into IR blocks. There is no standalone `PSXRecomp.Recompiler` project and no host code generation yet — memory operations, control flow, optimization, and the executable vertical slice are separate milestones. The CPU/decoder work above is foundational to it, not a substitute for it.
